...
// ─── Data Quality Filter ───
// Filters out HOAs, commercial properties, and garbage data
...
const MIN_VALID_RENT = 200;   // Below $200/mo is not a real apartment
const MAX_VALID_RENT = 15000;  // Above $15k/mo is likely data corruption

function isValidApartment(p: any): boolean {
  // Must have a rent in a reasonable range
  const rent = p.min_rent || p.max_rent || 0;
  if (rent < MIN_VALID_RENT || rent > MAX_VALID_RENT) return false;

  // Filter out non-apartment entities by name
  const name = (p.name || '').toLowerCase();
  if (NON_APARTMENT_KEYWORDS.some(kw => name.includes(kw))) return false;

  // Filter out entries with no real address
...
export function registerScrapedPropertyRoutes(app: Express): void {
  app.get("/api/scraped-properties", async (_req: Request, res: Response) => {
    try {
      ...
      // Filter out non-apartments and garbage data
