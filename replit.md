...
- **Scraped Properties API**: `server/routes/scraped-properties.ts` connects to Supabase (using `SUPABASE_URL` and `SUPABASE_ANON_KEY`) to fetch scraped apartment data. Routes: `GET /api/scraped-properties` (all listings), `GET /api/scraped-properties/:id` (single property), `GET /api/scraped-properties/stats` (aggregate stats). Backend maps Supabase field names (current_price, bedrooms, bathrooms, listing_url) to frontend-expected names (min_rent/max_rent, bedrooms_min, bathrooms_min, website_url). Frontend: `src/lib/savings-calculator.ts` for deal scoring and savings breakdowns.
- **Google Places Photo Integration**: `server/services/places-photo.ts` uses Google Places Text Search API (New) to find apartment complex photos by address. Photos are cached in-memory (grouped by address for efficiency) and persisted to Supabase's `image_url` column. Lookups are per-complex, not per-unit, minimizing API calls.
