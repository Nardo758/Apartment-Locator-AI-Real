...
- **SerpApi Enrichment Pipeline**: Enriches scraped properties with direct website URLs (replacing aggregator links from apartments.com, apartmentlist.com, etc.) via Google Search. Admin endpoint at `/api/admin/enrich/serp`. Stores direct URLs in `direct_website_url` column, preserving original `listing_url`. Skips amenity enrichment when existing count >= 5 to preserve rich ApartmentList data.
